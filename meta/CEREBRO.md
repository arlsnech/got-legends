...
> **Adaptado ao GoT Legends Build Planner** a partir do template genérico do KCM v1.73.0 (nicho Desenvolvimento) em 2026-07-22. As seções «Convenções», «Build e validação deste projeto», «Ao receber um template-update do KCM» e «Kit do Claude Code» foram reescritas para descreverem o que este projeto DE FATO faz — ver DEC-007 a DEC-010.
...
## Convenções (deste projeto — substituem o padrão genérico do kit)

- **Código:** funções, componentes e variáveis em `camelCase` inglês (`computeStats`, `getStatGroups`, `linkedClass`).
- **IDs de domínio:** `snake_case`, em PT-BR, espelhando `src/data.js` (`ronin_breath`, `gw_kunai`, `class_samurai`). Exceção DELIBERADA ao "tudo em inglês": os IDs são a chave que liga `data.js` ↔ `logic.js` ↔ `icons.js`; renomeá-los quebra o mapeamento de ícones em silêncio. **Não "corrija" para inglês.**
- **Comentários:** PT-BR. Blocos de seção no formato `// ─── Nome ───` (é por eles que se navega o `App.jsx` monolítico).
- **Commits:** Conventional Commits — `tipo(escopo): descricao` (`feat`, `fix`, `docs`, `refactor`, `chore`) — com a mensagem **SEM acento**, porque o CMD do Windows os corrompe. Ver DEC-010.
- **Estilo:** não há linter ativo. A régua é a consistência com o código existente. Legibilidade primeiro; performance só se medida.
- **Idioma do produto:** PT-BR por padrão, com toggle EN. Todo texto de UI existe nos DOIS idiomas — nunca acrescente string monolíngue.

## Build e validação deste projeto

- `npm run dev` — servidor Vite em `localhost:5173`.
- `npm run build` — build de produção em `dist/`. **Rode antes de commitar mudança de código.**
- `npm run preview` — serve o `dist/` gerado, para conferir o build.
- `npm run deploy` — build + publicação no GitHub Pages via `gh-pages`. Só sob pedido explícito.
- **Não há suíte de testes.** A rede é o build + a conferência visual. Por isso o princípio 18 («sinaliza o que testar») é obrigatório aqui: toda mudança de código sai acompanhada da lista do que abrir e olhar na tela (caso feliz, borda, regressão provável).
- Mudança apenas em `meta/` NÃO precisa de build; a rede é o `git diff`.
...
> **Regra deste projeto (DEC-008): convenção genérica que colide com prática em vigor vale como SUGESTÃO, não como ordem.** O CEREBRO e as INSTRUCOES que o kit gera são texto de nicho, escritos sem conhecer esta obra. Quando uma linha deles bate de frente com algo que o projeto já faz — o caso canônico: «nomes de variáveis em inglês» × os IDs `snake_case` PT-BR de `data.js` —, o assistente **não refatora o projeto para caber no template**. Faz o inverso: adapta o CEREBRO e as Instruções para descreverem o que o projeto de fato faz, aponta a colisão ao usuário, e registra o desvio no DECISIONS + na seção «Feedback para o Kit» do IDEAS. Adotar o template ao pé da letra num projeto em andamento é regressão disfarçada de atualização.
>
...
## Kit do Claude Code — instalado neste repo

Os arquivos de arranque do Claude Code **já vivem na raiz deste repositório** (instalados em 2026-07-22, ver DEC-007). Não são conteúdo deste arquivo; ficam listados aqui só para o assistente saber que existem e o que cada um faz.
...
`CLAUDE.md` e a pasta `.claude/` ficam na **raiz do repo** e são **versionados** — o `.gitignore` traz a cláusula que proíbe ignorá-los. Formato atual (2026): os comandos são **Skills** em `.claude/skills/<nome>/SKILL.md` (o `.claude/commands/*.md` legado ainda funciona, mas Skills tem precedência).

*Adaptado do template do KCM v1.73.0 para o GoT Legends Build Planner em 2026-07-22. Este arquivo é do projeto: edite à vontade — mas registre o desvio no DECISIONS.*
